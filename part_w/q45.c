/* q45: kth smallest element in BST via inorder traversal with counter. */
#include <stdio.h>
#include <stdlib.h>
...
static int kth_smallest(struct Node *r, int *counter, int k, int *result) {
    if (!r) return 0;
    if (kth_smallest(r->left, counter, k, result)) return 1;
    (*counter)++;
    if (*counter == k) { *result = r->data; return 1; }
    return kth_smallest(r->right, counter, k, result);
}
...
int main(void) {
    ...
    for (int k = 1; k <= n; ++k) {
        int c = 0, res = -1;
        kth_smallest(root, &c, k, &res);
        printf("k=%d smallest = %d\n", k, res);
    }
    free_tree(root);
    return 0;
}